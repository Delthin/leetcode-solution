...
给你一个 下标从 0 开始的 整数数组 prices ，其中 prices[i] 表示你购买第 i + 1 个水果需要花费的金币数目。 
 ...
 Related Topics 队列 数组 动态规划 单调队列 堆（优先队列） 👍 67 👎 0
...
public:
    int minimumCoins(vector<int>& prices) {
        ...
        dq.emplace_back(n, 0);
        for (int i = n; i > 0; i--) {
            while (!dq.empty() && dq.back().first > i * 2) {
                dq.pop_back();
            }
            int f = prices[i - 1] + dq.back().second;
            while (!dq.empty() && dq.front().second >= f) {
                dq.pop_front();
            }
            dq.emplace_front(i - 1, f);
        }
